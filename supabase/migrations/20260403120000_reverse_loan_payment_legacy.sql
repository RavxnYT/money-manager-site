-- Backfill loan_payments.transaction_id when the ledger row exists but the link
-- column was never set (older app/DB versions).
update public.loan_payments lp
set transaction_id = t.id
from public.transactions t
where lp.transaction_id is null
  and t.user_id = lp.user_id
  and public.workspace_matches(t.organization_id, lp.organization_id)
  and t.source_type = 'loan_payment'
  and t.source_ref_id = lp.id;

-- Reverse loan payment: resolve transaction via loan_payments.transaction_id,
-- then source_ref (integrity era), then legacy create_transaction rows (no source).
